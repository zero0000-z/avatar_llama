...
if int(question)==1:
    ...
    video_label = [line for line in video_path.split(r"/") if 'mp4' in line][0].replace(".mp4", "")
    video_label_ = video_label.split("-")
    print('video label: ', video_label_)
    chat_state = None
    ...
    lima = read_json(r'/code/SinMDM_stuff/LLaMA-Factory/data/lima_rule_generate_70b_test.json')
    data1 = generate_content(lima, video_path=video_label)
    #get needed to change lima.jason

    #for i, text_input in enumerate(input_sentence_1):
    #   if len(video_label_)==1:
    #      text_input = text_input.format(video_label_[0], "")
    # else:
        #    text_input = text_input.format(video_label_[0], video_label_[1])

    output_path = "zoo_300k/zoo_300k_all_llama13B.json"
    write_json(data1, output_path)
    ...
    video_path_list = get_filepath(video_path_root)
    for video_path in video_path_list[:]:
        if 'AnacondaALL' in video_path :
            continue
        else:
            filelist = get_mp4_files(video_path)
            animal = video_path.split('/')[-1]
            data_all = []
            count = 0
            ...
            print('spending time is :', (time.time()-start_time)/count)
            # exists_create_file(output_path)
            output_file = output + "/" + animal + "_llama13B.json"
            write_json(data_all, output_file)
else:
    print('please input correct the number. "1 is only video path", "2 is many videos path."')
